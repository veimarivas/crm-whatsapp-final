...
- **Caché de la oferta envenenado — afectaba también a la IA (2026-08-07)**. `programasCacheadas()` guardaba la `Collection` de `stdClass` en `Cache`. Según el backend, al leer de vuelta PHP devolvía un **`__PHP_Incomplete_Class`** en lugar de la Collection: escribir funcionaba y leer no, así que **el fallo aparecía recién en el segundo request** (el primero es cache miss). `collect()` sobre ese objeto contaba sus campos internos como items — de ahí el síntoma «3 programas» con 11 reales, y el `Attempt to read property "id" on string`.
    - **No era solo de las plantillas**: `OfertaAcademica::contextoPara()` —el contexto que la IA usa en CADA mensaje— lee ese mismo caché. La IA se quedaba sin catálogo salvo en el primer request de cada ventana de 5 minutos.
    - Arreglo en la raíz: se cachean **arrays**, no objetos (`(array) $p` al guardar, `(object) $fila` al leer). Un array de escalares se reconstruye igual en cualquier backend. Clave nueva `esam_datos:programas_v2` para ignorar las entradas viejas envenenadas.
    - `Plantillas` además filtra filas no utilizables antes de generar y, si descarta todo, avisa en pantalla con la solución (`php artisan cache:clear`) en vez de un error de PHP sin contexto.
    - **Comando `wacrm:diagnosticar-plantillas`** (`--fresh` limpia los cachés): recorre el camino de la petición web paso a paso —conexión, consulta directa, consulta cacheada, colección que ve el generador, recetas, galería— y dice dónde se pierde. Fue lo que permitió encontrarlo: `wacrm:sync-oferta-academica --dry-run` consulta directo y por eso daba OK siempre.
- Sin migraciones. Tests en `PlantillasOfertaTest` (13, con dobles de `OfertaAcademica` — la BD académica no está en el entorno de tests), incluidos los del 500 y el del caché corrupto; suite total **364/364 (1499)**.
